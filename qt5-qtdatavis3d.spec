...
Version: 5.9.0
%if "%{beta}" != "%{nil}"
Source0: http://download.qt.io/development_releases/qt/%(echo %{version}|cut -d. -f1-2)/%{version}-%{beta}/submodules/qtdatavis3d-opensource-src-%{version}-%{beta}.tar.xz
Release: 0.%{beta}.1
%else
...
%prep
%if "%{beta}" != "%{nil}"
%setup -qn qtdatavis3d-opensource-src-%{version}-%{beta}
%else
